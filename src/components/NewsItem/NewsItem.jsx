...
const NewsItem = ({ article, location }) => {
  ...
  const src = article.id === 'clrds6uk6if5d0but4mf0j8s2' ? 'https://goksirgornagrupa.pl/turystyka/' : article.id === 'clruv2b635onz0bw52fn7lohx' ? 'https://itgrudziadz.pl/en/must-see-monuments/' : page
  const isOutsidePage = (article.id === 'clrds6uk6if5d0but4mf0j8s2' || article.id === 'clruv2b635onz0bw52fn7lohx') ? true : false
  ...
  return (
    <div className={css.articleContainer}>
      ...
      <article className={css.articleItem}>
        <h3><Link target={isOutsidePage ? '_blank':'_self'} className={css.articleRef} to={src} state={{from: location}}>{article.title}</Link></h3>
      </article>
    </div>
  )
}
